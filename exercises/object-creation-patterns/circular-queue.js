...
// When an object is added to this circular queue, it is added to
...
// the buffer size with an argument provided to the constructor,
...
// null (however, null may be used to designate empty spots in
//   the buffer).
...
class CircularQueue {
  ...
  enqueue(value) {
    if (this.buffer[this.nextPosition] !== null) { //if not empty
  ...
  increment(position) {
    return (position === this.buffer.length - 1) ? 0 : position + 1;
  }
}
